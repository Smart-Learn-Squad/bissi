...
var QWebChannel = function(transport, initCallback) {
    ...
    this.handlePropertyUpdate = function(message) {
        var data = message.data;
        // Qt6 may send as array [{object, signals, properties}] or object map
        if (Array.isArray(data)) {
            data.forEach(function(entry) {
                var object = channel.objects[entry.object];
                if (object) object.propertyUpdate(entry.signals, entry.properties);
            });
        } else {
            for (var objectName in data) {
                if (!data.hasOwnProperty(objectName)) continue;
                var object = channel.objects[objectName];
                if (object) object.propertyUpdate(data[objectName].signals, data[objectName].properties);
            }
        }
    };
    ...
    this.exec({ type: QWebChannelMessageTypes.init }, function(data) {
        // Qt6 sends data as an object map {name: classInfo}, not an array
        for (var objectName in data) {
