...
from core.path_validator import is_path_allowed

MAX_WORD_SIZE = 50 * 1024 * 1024  # 50 Mo
...
class DocxAgent:
    ...
    def __init__(self, file_path: str):
        ...
        if not is_path_allowed(file_path):
            raise ValueError(f"Accès interdit : {file_path}")
        
        path = Path(file_path)
        if path.exists() and path.stat().st_size > MAX_WORD_SIZE:
            raise ValueError(f"Fichier trop volumineux (> 50 Mo) : {file_path}")
    ...
    def save(self, new_file_path: str = None) -> None:
        ...
        """
        if new_file_path:
            if not is_path_allowed(new_file_path):
                raise ValueError(f"Accès interdit : {new_file_path}")
            self.document.save(new_file_path)
        else:
            if not is_path_allowed(self.file_path):
                raise ValueError(f"Accès interdit : {self.file_path}")
...
def write_document(file_path: str, content: str, append: bool = False) -> bool:
    ...
    if append and os.path.exists(file_path):
        if Path(file_path).stat().st_size > MAX_WORD_SIZE:
            raise ValueError(f"Fichier trop volumineux (> 50 Mo) : {file_path}")
